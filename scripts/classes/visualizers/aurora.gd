...
func _draw():
	for index: int in element_count:
		var color: Color = responsive_color(index)
		...
		var rectangle: Rect2 = Rect2(
			...
			(elements[index].heightLerped * 1.5) + (size.y/25)  # Height
		)
		...
		var rectangle2: Rect2 = Rect2(
			...
			((elements[index].heightLerped * 3) + (size.y * 0.8)) / 2	# Height
		)
		...
		var rectangle3: Rect2 = Rect2(
			...
			((elements[index].heightLerped * 3) + size.y) / 1.5 	# Height
		)
